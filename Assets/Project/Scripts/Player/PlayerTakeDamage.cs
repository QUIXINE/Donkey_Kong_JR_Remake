...
{
    sealed public class PlayerTakeDamage : MonoBehaviour
    {
        ...
        public void TakeDamage()
        {
            ...
            if(PlayerPrefs.GetInt("Current_Player") == 1)
            {
                if(Player01Health.Instance.Health > 0 && Player01Health.Instance.Health > 1)
                {
                    ...
                    EnemyCollidePlayer[] enemies = FindObjectsOfType<EnemyCollidePlayer>();   //Script that makes enemy moves
                    Fruit[] fruits = FindObjectsOfType<Fruit>();
                    foreach(EnemyCollidePlayer enemy in enemies)
                    {
                        enemy.enabled = false;
                    }
