...
namespace ScoreManagement
{
    ...
    {
        ScoreRankDisplay score_Rank_Display;
        ...
        void Start()
        {
            score_Rank_Display = FindObjectOfType<ScoreRankDisplay>(); 
        }
            ...
            if(PlayerPrefs.GetInt("Player_Amount") == 1)
            {
                for(int i = 0; i < LoadAndSaveScore.player_list.Count; i++)
                {
                    if(PlayerPrefs.GetInt("Player01_Score") == LoadAndSaveScore.player_list[i].Score)  
                    {
                        ...
                        LoadAndSaveScore.player_list[i].Name = score_Rank_Display.players_Name[i].text;       // change name of player in list in Json w/ input name  
                        LoadAndSaveScore.SaveHighScores();
            ...
            else
            {
                if(PlayerPrefs.GetInt("Current_Player") == 1)
                {
                    for(int i = 0; i < LoadAndSaveScore.player_list.Count; i++)
                    {
                        ...
                        {
                            score_Rank_Display.players_Name[i].text = $"{SelectedLetter[0].text}{SelectedLetter[1].text}{SelectedLetter[2].text}";
                            LoadAndSaveScore.player_list[i].Name = score_Rank_Display.players_Name[i].text;
                            LoadAndSaveScore.SaveHighScores();
                            if(LoadAndSaveScore.player_list.Any(player => PlayerPrefs.GetInt("Player02_Score") == player.Score))
                            {
                                EraseLetter();
                ...
                else if (PlayerPrefs.GetInt("Current_Player") == 2)
                {
                    for(int i = 0; i < LoadAndSaveScore.player_list.Count; i++)
                    {
                        if(PlayerPrefs.GetInt("Player02_Score") == LoadAndSaveScore.player_list[i].Score)  
                        {
