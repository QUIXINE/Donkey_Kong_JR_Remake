...
sealed public partial class Player
{
    ...
    {
        Vector2 pos = obstacleCheckPos01.position;
 ...
 */
        ...
        if(Physics2D.OverlapBox(obstacleCheckPos01.position , new Vector2(xLengthObstacleCheck, yLengthObstacleCheck), 0, groundLayerMask))
        {
            return true;
        }
        else if (Physics2D.OverlapBox(obstacleCheckPos01.position , new Vector2(xLengthObstacleCheck, yLengthObstacleCheck), 0, boundaryLayerMask))
        {
            return true;
        }
        return false;
    ...
    {
        //check if player jump from under of vine
        return Physics2D.OverlapBox(groundCheckPosOnHead.position, new Vector3(xLengthCheckOnHead,0.05f,0), 0,groundLayerMask);
    }
    #endregion
