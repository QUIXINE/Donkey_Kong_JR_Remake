...
public partial class Player
{
    ...
    private void TwoHandedState()
    {
        ...
        else if (/* Input.GetKey(KeyCode.LeftArrow) */HorizontalOnVine < -0.32f && transform.rotation == Quaternion.Euler(0, -180, 0) && !canReach && isTwoHanded && canChangeToReach && !IsThereGroundAsObstacle())
        {
            if(IsThereBoundaryAsObstacle() && !IsThereKeyAsObstacle())
            {
                canReach = true;
